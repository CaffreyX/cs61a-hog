...
def roll_dice(num_rolls, dice=six_sided):
    ...
    if One == True:
        ...
        return total 
#     如果直接return 1， 而不设boolean value的话，当后面的function make_averaged call
#     roll_dice的时候，没有达到num_rolls次数，运算就会直接终止。 因而会直接影响到后面roll_dice的结果
# eg. >>> dice = make_test_dice(4,1,2,3,5)
#     >>> averaged_dice = make_averaged(roll_dice, 4)  
#     >>> averaged_dice(3, dice) 
# 不设boolean的结果就是return  (4,1),(2,3,5),(4,1),(2,3,5) 平均值得(1+10+1+10)/4=5.5 
# 设置boolean才能算出正确值 (4,1,2),(3,5,4),(1,2,3),(5,4,1) 平均值得 (1+12+1+1)/4=3.75
    # END PROBLEM 1
...
def more_boar(player_score, opponent_score):
    ...
    if opponent_score < 10:
        ...
        if player_score < 10:
            ...
            if opponent_score % 10 > player_score % 10:
                opponent_score //= 10
                player_score //= 10
                if opponent_score % 10 > player_score % 10:
                    return True
    ...
    def commentary(score0,score1,last_score=last_score,running_high=running_high):
        if who == 0:
            ...
            if gain > running_high:
                running_high = gain
                print("Player {0} has reached a new maximum point gain. {1} point(s)!".format(who,running_high))
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    for i in range(1,11):
        ...
        if max< averaged_dice(i,dice):
            max = averaged_dice(i,dice)
            num = i
